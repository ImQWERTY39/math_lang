use crate::{math2::Expression, Scope};
...
pub enum Tokens {
    ...
    OpenBrackets,
    CloseBrackets,
    Function(String, Box<Vec<Vec<Tokens>>>),
}
...
fn correct(tokens: Vec<Tokens>, scope: &Scope) -> Option<Vec<Tokens>> {
    ...
    while i < tokens.len() {
        if let Tokens::Subtract = tokens[i] {
            ...
            if scope
                .get(var_name)
                .is_some_and(|x| matches!(x, Expression::Function(_)))
            {
                i += 2;
                ...
                tokens_corrected.push(Tokens::Function(var_name.to_owned(), Box::new(arguments)));
                continue;
            } else {
                tokens_corrected.push(tokens[i].clone());
            }
